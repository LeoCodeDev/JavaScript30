const inventors = [
  ...
  // Array.prototype.filter()
    ...
    const inventorsTimeLife = []

    inventors.forEach(inventor => {
        inventorsTimeLife.push(inventor.passed - inventor.year)
    })

    const yearsTotal = inventorsTimeLife.reduce((a,b) => {
        return a + b
    })
    ...
    boulevards.forEach(boulevard => {
        e = boulevard.indexOf('de')
        if(e >= 0 ){
            de.push(boulevard)
        }
    })
    ...
    const people = [
      ...
      const elements = people.sort((a,b) =>{
          const [bFirst, bLast] = a.split(', ')
          const [aFirst, aLast] = b.split(', ')
    ...
    const items = data.reduce((obj, vehicle) =>{
        if(!obj[vehicle]){
            obj[vehicle] = 0
        }
        obj[vehicle]++
